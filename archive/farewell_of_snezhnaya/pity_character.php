...
function pity(){
    ...
    if ($row["pity_4s"] < 9){
        $updatePITY4s = "UPDATE pity_character SET pity_4s=pity_4s+1 WHERE id=$user_id";
    ...
    if ($row["pity_5s"] < 89){
        $updatePITY5s = "UPDATE pity_character SET pity_5s=pity_5s+1 WHERE id=$user_id";
    ...
    $updateROLL = "UPDATE pity_character SET roll=roll+1 WHERE id=$user_id";
