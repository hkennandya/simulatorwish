...
require_once "archive/wanderlust_invocation/pity_standard.php";
$number;

function wishStandard(){
    ...
    if($number <= 9430){
        ...
        $sql = "SELECT * FROM wanderlust_invocation WHERE id=$random";
        $result = $conn->query($sql);
        ...
        while($row = $result->fetch_assoc()) {
            ...
            $rownamereal = $conn->real_escape_string($row[$name]); //Agar nilai petik bisa diinput ke tabel sql
            $updateHistory = "INSERT INTO history_standard (id, name, tier, date) 
